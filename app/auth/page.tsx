...
export default function AuthPage() {
  ...
  const handleEmailPasswordAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Set authInProgress during login
      setAuthInProgress(true);
      console.log("[Auth] authInProgress = true");
      ...
      // The redirect logic in useEffect will handle routing
      // Don't manually navigate - let the auth state change trigger redirect
    } catch (err: any) {
      console.error('[Auth] Auth error:', err);
      // Clear authInProgress on error
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      // Use popup (not redirect) so we can track authInProgress
      await signInWithPopup(auth, provider);
      console.log(`[Auth] Google ${isSignUp ? 'sign up' : 'sign in'} completed`);
      // Don't set isLoading to false - authInProgress will be cleared by onAuthStateChanged
      // Don't call router.replace - redirect logic handles routing
    } catch (err: any) {
      console.error('[Auth] Google auth error:', err);
      setAuthInProgress(false); // Clear on error
  ...
  const handleFacebookAuth = async () => {
    ...
    setError('');
    try {
      // Facebook uses NextAuth, which will handle its own redirect
      // We don't need to set authInProgress for NextAuth flows
      ...
      // If redirect: true, we won't reach here
    } catch (err) {
